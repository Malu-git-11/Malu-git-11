...
// CARREGA O JSON E RENDERIZA TUDO
fetch('./src/data/saude.json')
    .then(res => res.json())
    .then(data => {
        const container = document.getElementById("conteudo-saude");
            ...
            categoria.equipamentos.forEach(eq => {
                ...
                if (eq.coords) {
                    L.marker(eq.coords).addTo(map)
                        .bindPopup(`<b>${eq.nome}</b><br>${eq.horario}`);
                }
        ...
        // PESQUISA (precisa ser registrada após renderizar)
